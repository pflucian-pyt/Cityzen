        ...
        Thread {
            try {
                ...
                val s = try {
                    ...
                    Log.w(TAG, "legatura obisnuita a esuat: ${prima.message}; încerc canalul 1")
                ...
                Log.i(TAG, "legat la ${d.name} / ${d.address}")
                ok(id, null)
            } catch (e: Exception) {
                Log.w(TAG, "conectare esuata: ${e.message}")
                inchideTacut()
