...
class PunteBle(
    ...
    fun cere_dispozitiv(id: Int) {
        principal.post { JurnalBt.scrie("BLE: pornesc scanarea"); scaneaza(id) }
    }

    @JavascriptInterface
    fun conecteaza(id: Int, adresa: String) {
        principal.post {
            JurnalBt.scrie("BLE: conectare cerută către $adresa")
    ...
    @JavascriptInterface
    fun raport(): String = JurnalBt.raport(activitate, "ble")
    ...
    private fun scaneaza(id: Int) {
        ...
        fun opreste(alesDirect: BluetoothDevice?) {
            ...
            JurnalBt.scrie("BLE: scanare terminată · ${gasite.size} aparate găsite")
            if (gasite.isEmpty()) {
